...
// Gravity function has taken inspiration of Newton's law of universal gravitation.
void gravity(uint32_t m1, uint32_t m2, struct bullet *bulletptr, int64_t a, int64_t b) {
	...
	// Gravitation will effect the bullets velocity
	bulletptr->vel.x += ax;
	bulletptr->vel.y += ay;

	// Maximum velocity for the bullets
